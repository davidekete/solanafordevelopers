...
The `Poll` account stores the core information about the poll, such as its description and voting window.
However, there are a few more fields it should hold; for instance, it should keep track of the poll’s creator
using their public key for access control checks, because, remember, only the poll’s creator can close the poll.
It should also track if the poll has been closed (before voting ended) and a count of the options the poll has for input validation, and the start and end <Term term="slot"/> for time tracking (more on this later).
...
If you take a look at the code block above, you’ll notice an additional field that we haven't covered yet, `desc_len`. To understand what this property does, first, you have to understand why the `poll_description` type is a fixed-length buffer instead of a variable-length string.
