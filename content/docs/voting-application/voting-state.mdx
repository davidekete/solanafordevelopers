...
The `Poll` account stores the core information about the poll, such as its description and voting window. However, there are a few more fields it should hold; for instance, it should keep track of the poll’s creator using their public key for access control checks, because, remember, only the poll’s creator can close the poll. It should also store the state of the poll (Open/Closed) and a count of the options the poll has for input validation (more on this later).
...
pub struct Poll {
    ...
    pub poll_stage: PollStage,// highlight this
...
Even more dangerously, an attacker could craft raw bytes that deserialize into a `Poll` whose `poll_creator` field coincidentally equals their own public key. Your access-control check would pass, and the attacker would obtain privileged powers over someone else’s poll.

The standard remedy is to prefix every serialized account with an eight-byte discriminator: a unique tag derived from the account type’s name (commonly the first eight bytes of its SHA-256 hash). The moment your program loads an account, it first checks the tag. If the tag matches the expected constant, deserialization proceeds; if not, the instruction fails immediately with `InvalidAccountData`. That single comparison eliminates type confusion, blocks the spoofing tricks described above, and gives you a clean migration path should you ever add new versions of a struct.