---
title: Voting Program Setup
description: Create the files and install the dependencies required to complete your voting program.
icon: Settings
---

import { Step, Steps } from "fumadocs-ui/components/steps";
import { LinkPrev } from "../../../src/components/link-hover.tsx";


<Steps>
  <Step>
    Initialize a new Rust project by running the command below:

    ```bash title="shell"
    cargo init voting_program --lib
    ```

  </Step>

  <Step>
`cd` into the created project directory and run the commands create the files and folders required for this project:
```bash title="shell"
mkdir state enums
touch instructions.rs errors.rs processor.rs
```
</Step>
<Step>
Run the command below to install the required crates:

```bash title="shell"
cargo add solana-program@2.3.0 solana-system-interface@1.0.0 borsh@1.5.7 thiserror@2.0.12
```

</Step>

</Steps>

The unfamiliar dependencies you installed above include:

- <LinkPrev href={"https://docs.rs/thiserror/latest/thiserror/"}>`thiserror`</LinkPrev>: This crate the creation of custom error types. You will need it because this program will feature some custom errors.
