...
```
Repeat the formula for `PollOption` and `VoteRecord`. With these constructors in place, every account your program touches is type-safe, size-accurate, and immune to stray bytes.