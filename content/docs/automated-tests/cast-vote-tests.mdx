...
This test verifies that the same voter cannot cast two votes on the same poll. After the first successful vote, a second `CastVote` transaction from the same voter must fail because the `VoteRecord` PDA for `(poll, voter)` already exists.
